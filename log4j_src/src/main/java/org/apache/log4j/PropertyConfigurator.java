 ...
 * 
 * <P>
 * As of log4j version 0.8.5, at class initialization time class, the file <b>log4j.properties</b> will be searched from
 * the search path used to load classes. If the file can be found, then it will be fed to the
...
public class PropertyConfigurator implements Configurator {
	 ...
	 * 
	 * <p>
	 * The level value can consist of the string values OFF, FATAL, ERROR, WARN, INFO, DEBUG, ALL or a
	 * <em>custom level</em> value. A custom level value can be specified in the form level#classname. By default the
	 * repository-wide threshold is set to the lowest possible value, namely the level <code>ALL</code>.
	 * </p>
	 * 
	 * 
	 ...
	 * <p>
	 * The level value can consist of the string values OFF, FATAL, ERROR, WARN, INFO, DEBUG, ALL or a
	 * <em>custom level</em> value. A custom level value can be specified in the form <code>level#classname</code>.
	 * 
	 * <p>
   ...
   */
	...
	/*
	 * void configureOptionHandler(OptionHandler oh, String prefix, Properties props) { String[] options =
	 * oh.getOptionStrings(); if(options == null) return;
	 * 
	 * String value; for(int i = 0; i < options.length; i++) { value = OptionConverter.findAndSubst(prefix + options[i],
	 * props); LogLog.debug( "Option " + options[i] + "=[" + (value == null? "N/A" : value)+"]."); // Some option
	 * handlers assume that null value are not passed to them. // So don't remove this check if(value != null) {
	...
	private void parseErrorHandler(final ErrorHandler eh, final String errorHandlerPrefix, final Properties props,
		...
		if (loggerName != null) {
			Logger logger = (loggerFactory == null) ? hierarchy.getLogger(loggerName) : hierarchy.getLogger(loggerName,
					loggerFactory);
			eh.setLogger(logger);
		}
	...
	void parseAppenderFilters(Properties props, String appenderName, Appender appender) {
		...
		while (g.hasMoreElements()) {
			...
			if (clazz != null) {
				LogLog.debug("Filter key: [" + key + "] class: [" + props.getProperty(key) + "] props: "
						+ filters.get(key));
