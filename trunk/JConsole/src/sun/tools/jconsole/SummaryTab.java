...
class SummaryTab extends Tab {
			...
			protected void done() {
				try {
					...
					if (result != null) {
						...
						if (overviewPanel != null && result.upTime > 0L
	...
	synchronized Result formatSummary() {
		...
		try {
			...
			OperatingSystemMXBean osMBean = proxyClient
					.getOperatingSystemMXBean();
			com.sun.management.OperatingSystemMXBean sunOSMBean = proxyClient
					.getSunOperatingSystemMXBean();

			append("<tr><td colspan=4>");
			append("<center><b>" + getText("SummaryTab.tabName")
					+ "</b></center>");
			String dateTime = headerDateTimeFormat.format(System
					.currentTimeMillis());
			...
			{ // VM info
				...
				append("Virtual Machine",
						getText("SummaryTab.vmVersion", rmBean.getVmName(),
								rmBean.getVmVersion()));
				...
				if (sunOSMBean != null) {
					...
					append("Process CPU time",
							formatNanoTime(result.processCpuTime));
				}

				if (cmpMBean != null) {
					append("JIT compiler", cmpMBean.getName());
					append("Total compile time",
							cmpMBean.isCompilationTimeMonitoringSupported() ? formatTime(cmpMBean
			...
			{ // Threads and Classes
				...
				String[] strings1 = formatLongs(tlCount, tpCount, tdCount,
						ttCount);
			...
			{ // Memory
				...
				// System.out.println("堆内存使用大小："+u.getUsed()/1024+"Kb");
				// ///////堆内存；
				...
				append(newTable);
				Collection<GarbageCollectorMXBean> garbageCollectors = proxyClient
			...
			{ // Operating System info
				...
				if (sunOSMBean != null) {
					String[] kbStrings1 = formatKByteStrings(sunOSMBean
					...
					/*
					 * long
					 * usedMemorylong=(sunOSMBean.getTotalPhysicalMemorySize
					 * ()-sunOSMBean.getFreePhysicalMemorySize())/(1024);
					 * System.
					 * out.println("VM总物理内存-空闲物理内存："+usedMemorylong+"Kb"); //
					 * 不着调 MemoryUsage mu = null; MemoryUsage nu = null; mu =
					 * proxyClient.getMemoryMXBean().getNonHeapMemoryUsage(); nu
					 * = proxyClient.getMemoryMXBean().getHeapMemoryUsage();
					 ...
					 */
					System.out.println("分配的虚拟内存："
							+ sunOSMBean.getCommittedVirtualMemorySize() / 1024
							+ "Kb"); // 分配的虚拟内存
					long memoryTotalCommited = sunOSMBean
							.getCommittedVirtualMemorySize()
							/ 1024
			...
			{ // VM arguments and paths
				append(newTable);
				String args = "";
				java.util.List<String> inputArguments = rmBean
						.getInputArguments();
				...
				append("Boot class path",
						rmBean.isBootClassPathSupported() ? rmBean
								.getBootClassPath() : getText("Unavailable"), 4);
				append(endTable);
			}
		} catch (IOException e) {
			if (JConsole.isDebug()) {
	...
	private static class CPUOverviewPanel extends OverviewPanel {
		...
		CPUOverviewPanel() {
			super(getText("CPU Usage"), cpuUsageKey, cpuUsageName,
					Plotter.Unit.PERCENT);
		...
		public void updateCPUInfo(Result result) {
			if (prevUpTime > 0L && result.upTime > prevUpTime) {
				...
				float cpuUsage = Math.min(99F, elapsedCpu
						/ (elapsedTime * 10000F * result.nCPUs));
				...
				getInfoLabel().setText(
						getText(cpuUsageFormat, String.format("%."
								+ CPU_DECIMALS + "f", cpuUsage)));
			}
