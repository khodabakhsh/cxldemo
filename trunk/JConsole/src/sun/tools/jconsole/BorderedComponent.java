...
public class BorderedComponent extends JPanel implements ActionListener {
	 ...
	 * <p>
	 * If the border property value is not specified in the constuctor or by invoking the appropriate set method, the
	 * property value will be defined by the current look and feel, using the following property name in the Defaults
	 * Table:
	 * <ul>
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
			...
			if (border != null) {
				if (grooveRect.y > compLoc.y - ascent) {
					...
					clipRect.setBounds(saveClip);
					if (computeIntersection(clipRect, compLoc.x + labelDim.width + 1, y, x + width
							- (compLoc.x + labelDim.width + 1), height)) {
						g.setClip(clipRect);
						border.paintBorder(c, g, grooveRect.x, grooveRect.y, grooveRect.width, grooveRect.height);
					}
