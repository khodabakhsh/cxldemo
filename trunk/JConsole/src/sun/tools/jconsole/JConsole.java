...
public class JConsole extends JFrame implements ActionListener,
		...
		if (System.getProperty("swing.defaultlaf") == null) {
			...
			if (systemLaF.equals("com.sun.java.swing.plaf.gtk.GTKLookAndFeel")
				...
				try {
					...
					System.err.println(Resources.getText("JConsole: ",
							e.getMessage()));
				}
			}
		}
	...
	private final static String title = Resources
			.getText("Java Monitoring & Management Console");
	...
	public JConsole(boolean hotspot) {
		...
		if (hotspot) {
			...
			hotspotMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_H,
	...
	private class WindowMenu extends JMenu {
		...
		private void add(VMInternalFrame vmIF) {
			...
			String text = SwingUtilities.layoutCompoundLabel(this,
					getGraphics().getFontMetrics(getFont()), indexString + " "
							+ vmName, null, 0, 0, 0, 0, viewR, iconR, textR, 0);
		...
		private void remove(VMInternalFrame vmIF) {
			...
			JMenuItem mi = (JMenuItem) vmIF
					.getClientProperty("JConsole.menuItem");
			remove(mi);
	...
	// Call on EDT
	void addUrl(final String url, final String userName, final String password,
			final boolean tile) {
		new Thread("JConsole.addUrl") {
			public void run() {
				try {
					addProxyClient(
							ProxyClient.getProxyClient(url, userName, password),
							tile);
	...
	// Call on EDT
	void addHost(final String hostName, final int port, final String userName,
			final String password, final boolean tile) {
		new Thread("JConsole.addHost") {
			public void run() {
				try {
					addProxyClient(ProxyClient.getProxyClient(hostName, port,
							userName, password), tile);
				} catch (final IOException ex) {
					dbgStackTrace(ex);
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							showConnectDialog(null, hostName, port, userName,
									password, errorMessage(ex));
						}
					});
				}
			}
	...
	// Call on worker thread
	private void failed(final Exception ex, final String url,
			final String userName, final String password) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				dbgStackTrace(ex);
				showConnectDialog(url, null, -1, userName, password,
						errorMessage(ex));
			}
		});
	}
	...
	private void showConnectDialog(String url, String hostName, int port,
			String userName, String password, String msg) {
		if (connectDialog == null) {
			connectDialog = new ConnectDialog(this);
		}
		connectDialog.setConnectionParameters(url, hostName, port, userName,
				password, msg);
	...
	private static void mainInit(final List<String> urls,
			final List<String> hostNames, final List<Integer> ports,
			final List<LocalVirtualMachine> vmids,
			final ProxyClient proxyClient, final boolean noTile,
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				for (int i = 0; i < hostNames.size(); i++) {
					jConsole.addHost(hostNames.get(i), ports.get(i), null,
							null, (i == hostNames.size() - 1) ? !noTile : false);
				}

				for (int i = 0; i < urls.size(); i++) {
					jConsole.addUrl(urls.get(i), null, null,
							(i == urls.size() - 1) ? !noTile : false);
				}

				for (int i = 0; i < vmids.size(); i++) {
					jConsole.addVmid(vmids.get(i),
							(i == vmids.size() - 1) ? !noTile : false);
				}

				if (vmids.size() == 0 && hostNames.size() == 0
						&& urls.size() == 0) {
		...
		for (int i = argIndex; i < args.length; i++) {
			...
			if (isValidRemoteString(arg)) {
				...
				if (!isLocalAttachAvailable()) {
					System.err
							.println("Local process monitoring is not supported");
					return;
				}
				try {
	...
	private static void initPluginService(String pluginPath) {
		if (pluginPath.length() > 0) {
			try {
				...
				ServiceLoader<JConsolePlugin> plugins = ServiceLoader.load(
						JConsolePlugin.class, pluginCL);
				// validate all plugins
				for (JConsolePlugin p : plugins) {
					if (isDebug()) {
						System.out.println("Plugin " + p.getClass()
								+ " loaded.");
					}
				}
	...
	private static class FixedJRootPane extends JRootPane {
		...
		/**
		 * The revalidate method seems to be the only one that gets called
		 * whenever there is a change of L&F or change of theme in Windows L&F
		 * and GTK L&F.
		 */
		@Override
