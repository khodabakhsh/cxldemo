...
public class Plotter extends JComponent implements Accessible, ActionListener,
	 ...
	 * "时间范围"的各个名称
	 */
	static final String[] rangeNames = { Resources.getText(" 1 min"),
			Resources.getText(" 5 min"), Resources.getText("10 min"),
			Resources.getText("30 min"), Resources.getText(" 1 hour"),
			Resources.getText(" 2 hours"), Resources.getText(" 3 hours"),
			Resources.getText(" 6 hours"), Resources.getText("12 hours"),
			Resources.getText(" 1 day"), Resources.getText(" 7 days"),
			Resources.getText(" 1 month"), Resources.getText(" 3 months"),
			Resources.getText(" 6 months"), Resources.getText(" 1 year"),
			Resources.getText("All") };

	/**
	 * "时间范围"对应的各个分钟数
	 */
	static final int[] rangeValues = { 1, 5, 10, 30, 1 * 60, 2 * 60, 3 * 60,
			6 * 60, 12 * 60, 1 * 24 * 60, 7 * 24 * 60, 1 * 31 * 24 * 60,
			3 * 31 * 24 * 60, 6 * 31 * 24 * 60, 366 * 24 * 60, -1 };
	...
	/**
	 * {@link Sequence}的集合，而同一个{@link Sequence}里保存了它对应坐标线的y轴坐标值
	 */
	...
	/**
	 * 选择文件对话框，用于右键菜单项  "将数据另存为..."的功能
	 */
	private JFileChooser saveFC;
    /**
     * 当值大于0时，表示x轴的起点刻度到终点刻度的分钟数
     */
	...
	/**
	 * 包含坐标轴(不包含刻度)的边框，默认赋值为{@link BevelBorder}实例。即包含左边宽、上边框的矩形图形
	 */
	private Border border = null;
	/**
	 * 代表整个坐标轴(包含刻度)的{@link Rectangle}
	 */
	...
	public void createSequence(String key, String name, Color color,
			boolean isPlotted) {
	...
	public JPopupMenu getComponentPopupMenu() {
		if (popupMenu == null) {
			...
			timeRangeMenu = new JMenu(
					Resources.getText("Plotter.timeRangeMenu"));
	...
	private void saveDataToFile(File file) {
		try {
			...
			JOptionPane.showMessageDialog(
					this,
					getText("FileChooser.savedFile", file.getAbsolutePath(),
							file.length()));
			...
			JOptionPane.showMessageDialog(this,
					getText("FileChooser.saveFailed.message", path, msg),
	...
	public void paintComponent(Graphics g) {
		...
		boolean bgIsLight = (bg.getRed() > 200 && bg.getGreen() > 200 && bg
				.getBlue() > 200);
		...
		if (border == null) {
			...
			// over and over.
			border = new BevelBorder(BevelBorder.LOWERED, getBackground()
					.brighter().brighter(), getBackground().brighter(),
					getBackground().darker().darker(), getBackground().darker());
		}
		...
		//变量tickInterval表示x轴刻度之间的毫秒间隔，【PS:tick在坐标轴中表示刻度】
			...
			if (seq.isPlotted && seq.size > 0) {
				...
				for (int pass = 0; pass < 2; pass++) {
					...
					for (int p = start; p < nValues; p += pointsPerPixel) {
						...
						int x2 = (int) (w
								* (times.time(p) - (t1 - viewRangeMS)) / viewRangeMS);
						long v2 = seq.value(p);
						if (v2 >= vMin && v2 <= vMax) {
				...
				if (v >= vMin && v <= vMax) {
					...
					g.fillPolygon(new int[] { x + 2, x + 6, x + 6 }, new int[] {
							y, y + 3, y - 3 }, 3);
				}
				g.setColor(fg);
			}
		...
		for (int i = 0; i < nLists; i++) {
			...
			if (seq.isPlotted && seq.size > 0) {
				...
				if (v >= vMin && v <= vMax) {
					...
					if (checkRightMargin(valWidth)
							|| checkRightMargin(legendWidth)) {
						// Wait for next repaint
						return;
					}
					//以下两句，分别画坐标线最右方的文字和数值
					g.drawString(legend, x + 17,
							Math.min(topMargin + h, y2 + 3 - 10));
					g.drawString(curValue, x + 17,
							Math.min(topMargin + h + 10, y2 + 3));
     ...
     * @param w x轴宽度
     * @param hGap
     * @param viewRangeMS x轴的起点刻度到终点刻度的毫秒数
     * @return x轴刻度之间的毫秒间隔
     */
	...
	private static synchronized Stroke getDashedStroke() {
		if (dashedStroke == null) {
			dashedStroke = new BasicStroke(1.0f, BasicStroke.CAP_BUTT,
					BasicStroke.JOIN_MITER, 10.0f, new float[] { 2.0f, 3.0f },
					0.0f);
		}
		return dashedStroke;
	}
	...
	private static Object extendArray(Object a1) {
		...
		Object a2 = Array.newInstance(a1.getClass().getComponentType(), n
	...
	private static class Sequence {
		...
		Object values = new byte[ARRAY_SIZE_INCREMENT
				+ (int) (Math.random() * 100)];
		...
		public void approveSelection() {
			...
			if (file != null) {
				...
				if (filter != null && filter instanceof FileNameExtensionFilter) {
					String[] extensions = ((FileNameExtensionFilter) filter)
							.getExtensions();
			...
			if (seqs.size() > 0 && seqs.get(0).size > 0) {
				...
				for (Sequence seq : seqs) {
					if (seq.isPlotted) {
						...
						if (seq.size > 0) {
							if (unit == Unit.BYTES) {
								value = getText("Size Bytes",
										seq.value(seq.size - 1));
							} else {
								value = getFormattedValue(
										seq.value(seq.size - 1), false)
										+ ((unit == Unit.PERCENT) ? "%" : "");
							}
						}
						// Assume format string ends with newline
						keyValueList += getText(
								"Plotter.accessibleName.keyAndValue", seq.key,
								value);
					}
				}
				name += "\n" + keyValueList + ".";
