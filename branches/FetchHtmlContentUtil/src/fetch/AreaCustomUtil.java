 ...
 * 
 */
public class AreaCustomUtil {

	public static String getDetailUrl = "http://fengsu.supfree.net/up.asp";//
	public static String getListUrl = "http://fengsu.supfree.net/left.asp";//
	...
	public static Map<String, List<KeyValue>> getList(String id) {
		List<KeyValue> list = new ArrayList<AreaCustomUtil.KeyValue>();
		Document doc = null;
		try {
			doc = getDocument(getListUrl + "?id=" + URLEncoder.encode(id, GBK));
		...
		for (Element href : hrefsElements) {
			...
			list.add(new KeyValue(href.attr("href").substring("up.asp?id=".length()), href.text()));
			count++;
		}
		area_custom_map.put(id, list);
		return area_custom_map;
	}
	...
	public static <E> void main(String[] args) {
		...
		String nationString = "天津 重庆 上海 河北 山西 山东 河南 陕西 甘肃 青海 江苏 浙江 安徽 湖北 湖南 云南 贵州 四川 广东 福建 海南 江西 辽宁 吉林 黑龙江 广西 新疆 西藏 内蒙古 宁夏 台湾 香港 澳门";
		String[] nationArray = nationString.split(" ");
		System.out.println(nationString.split(" ").length);
		//		for (int i = 0; i < nationArray.length; i++) {
		//			Map<String, List<KeyValue>> map = getList(nationArray[i]);
		//			for (final KeyValue keyValue : map.get(nationArray[i])) {
		//				new Thread() {
		//					public void run() {
		//						CommonUtil.WriteFile(CommonUtil.writeFileBasePath + "\\各地风俗\\content_" + keyValue.getKey()
		//								+ ".txt", getDetail(keyValue.getKey()));
		//					};
		//				}.start();
		//
		//			}
		...
		for (int i = 0; i < nationArray.length; i++) {
			System.out.print(nationArray[i]+"\",\"");
		}
		
//		//生成语句
//		for (int i = 0; i < nationArray.length; i++) {
//			getList(nationArray[i]);
//		}
//
//		for (Entry<String, List<KeyValue>> entry: area_custom_map.entrySet()) {
...
//			
//			}
//			System.out.println("area_custom_map.put(\""+entry.getKey()+"\",list);");
//		}
	}
}