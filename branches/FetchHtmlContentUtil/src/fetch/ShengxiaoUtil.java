...
public class ShengxiaoUtil
{
	...
	private static Document getDocument(String url)
	{
		try
		{
			...
			return Jsoup
					...
					.header("Connection", "keep-alive").header("Cache-Control",
							"max-age=0").header("Accept-Encoding",
							"gzip, deflate").get();
		} catch (IOException e)
		{
			e.printStackTrace();
	...
	public static String getShengxiaoDetail(String id)
	{
	...
	{
		String[] items = new String[] { "shu", "niu", "hu", "tu", "long",
				"she", "ma", "yang", "hou", "ji", "gou", "zhu" };
		for (int i = 0; i < items.length; i++)
		{
			CommonUtil.WriteFile("d:\\抓取内容\\生肖运势\\【" + (i+1) + "】" + items[i]
					+ ".txt", ShengxiaoUtil.getShengxiaoDetail(items[i]));
		}
	}
}
