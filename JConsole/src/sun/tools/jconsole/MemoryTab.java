...
class MemoryTab extends Tab implements ActionListener, ItemListener {
	 ...
	 * 
	 * Center: controlPanel (FlowLayout) plotterChoice, timeComboBox
	 * 
	 * East: topRightPanel (FlowLayout) gcButton
	 * 
	 * Center: plotterPanel
	 * 
	 * Center: plotter
    ...
    /**
     * {@link #gcButton} 的响应，以后台线程的方式进行gc
     */
	public void gc() {
				...
				for (int i = 0; i < n; i++) {
					...
					try {
						if (plotter instanceof PoolPlotter) {
							...
							AttributeList al = proxyClient.getAttributes(objectName, new String[] { "Usage",
									"UsageThreshold" });
							if (al.size() > 0) {
	...
	private class PoolChart extends BorderedComponent implements Accessible, MouseListener {
		...
		public void paintComponent(Graphics g) {
			...
			for (int pc : new int[] { 0, 25, 50, 75, 100 }) {
				String str = pc + "% --";
				g.drawString(str, leftMargin - fm.stringWidth(str) - 4, height - bottomMargin - (pc * barHeight / 100)
						+ fontDescent + 1);
			}
	...
	private static class MemoryOverviewPanel extends OverviewPanel {
		...
		private void updateMemoryInfo(long used, long committed, long max) {
			getInfoLabel().setText(
					getText(infoLabelFormat, formatBytes(used, true), formatBytes(committed, true),
